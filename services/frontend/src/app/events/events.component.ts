...
export class EventsComponent implements OnInit {
  ...
  constructor(private eventService: EventService, private transferService: TransferService, private fileService: FileService) { }
  ...
  deleteEventById(id: number) {
    this.eventService.deleteEventById(id).subscribe(
      () => {
        this.fetchedEvents = this.fetchedEvents.filter(event => event.id != id);
        this.transferService.setEvents(this.fetchedEvents);
      },
      (error) => {
        this.errorMessage = error;
