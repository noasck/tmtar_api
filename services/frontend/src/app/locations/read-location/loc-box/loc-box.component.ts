...
export class LocBoxComponent implements OnInit {
  ...
  constructor(private transferService: TransferService, private locationService: LocationService) {
  }
  ...
  deleteLocation(delOption, deleteId){
    if(delOption){
      this.locationService.deleteLocationByID(deleteId).subscribe(
        () => {
          this.allLocations = this.allLocations.filter(user => user.id != deleteId);
